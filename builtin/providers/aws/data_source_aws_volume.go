...
func dataSourceAwsVolumes() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceAwsVolumesRead,
...
func dataSourceAwsVolumesRead(d *schema.ResourceData, meta interface{}) error {
	...
	if len(filteredVolumes) > 1 {
		...
		log.Printf("[DEBUG] aws_volume - multiple results found and `most_recent` is set to: %t", recent)
		if recent {
